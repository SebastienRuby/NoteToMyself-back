...
class Meal {
    ...
    static async update (req, res){
        ...
        const query = 'UPDATE public.meal SET name=$1, slug=$2, photo_url=$3, favorite=$4, review=$5, meal_restaurant_id=$6, updated_at=$7 WHERE id=$8 RETURNING *';
        const values = [req.body.name, req.body.slug , req.body.photo_url , req.body.favorite , req.body.review , req.body.meal_restaurant_id , date , req.headers.id];
        try {
            const result = await client.query(query, values);
            res.json(result.rows[0]);
        }
