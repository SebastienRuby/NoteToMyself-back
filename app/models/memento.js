...
const memento = {
    ...
    async update (req, res){
            const query = 'UPDATE public.memento SET name=$1, content=$2, reminder=$3, memento_restaurant_id=$4  WHERE id=$5 RETURNING *';
            const values = [req.body.name, req.body.content , req.body.reminder , req.body.memento_restaurant_id , req.headers.id];
            try {
                const result = await client.query(query, values);
                res.json(result.rows[0]);
            }
